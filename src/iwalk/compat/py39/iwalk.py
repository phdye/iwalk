...
IGNORE_FILENAMES: List[str] = ['.gitignore', '.dockerignore', '.ignore']

def walk(
    root_dir: str,
    ignore_files: List[str] = IGNORE_FILENAMES,
...
def load_ignore_specs(root_dir: str, ignore_files: List[str]) -> Dict[str, PathSpec]:
    """
    Loads all supported ignore files and maps PathSpecs to directories.
    """
    spec_map: Dict[str, PathSpec] = {}
...
def is_ignored(path: str, spec_map: Dict[str, PathSpec]) -> bool:
...
def load_repo_exclude_patterns(root_dir: str) -> List[str]:
