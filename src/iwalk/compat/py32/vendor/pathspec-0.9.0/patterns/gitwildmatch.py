...
class GitWildMatchPattern(RegexPattern):
	...
	def pattern_to_regex(cls, pattern):
		...
		*pattern* (:class:`str` or :class:`bytes`) is the pattern to convert into a
		...
		if isinstance(pattern, str):
			...
			raise TypeError("pattern:{0} is not a unicode or byte string.".format(pattern))
		...
		regex    = None  # : Optional[str]
		include  = None  # : Optional[bool]
		...
		if pattern.startswith('#'):
			...
			# normalization.
			override_regex    = None  # : Optional[str] = None
			...
			if len(pattern_segs) == 2 and pattern_segs[0] == '**' and not pattern_segs[1]:
				...
				override_regex = '^.+(?P<{0}>/).*$'.format(_DIR_MARK)
			...
			if not pattern_segs:
				...
				raise GitWildMatchPatternError("Invalid git pattern: {0!r}".format(original_pattern))
				...
				for i, seg in enumerate(pattern_segs):
					if seg == '**':
						if i == 0 and i == end:
							...
							if is_dir_pattern:
								output.append('(?P<{0}>/).*'.format(_DIR_MARK))
							else:
								output.append('/.*'.format())
	...
	def _translate_segment_glob(pattern) :
		...
		*pattern* (:class:`str`) is the glob pattern.
		...
		if escape:
			raise ValueError("Escape character found with no next character to escape: {0!r}".format(pattern))
	...
	def escape(s) :
		...
		*s* (:class:`str` or :class:`bytes`) a filename or a string that you want to
		...
		if isinstance(s, str):
			...
			raise TypeError("s:{0} is not a unicode or byte string.".format(s))
...
# /\/\/\ Fails due existing registration in `util.register_pattern`:

# Force registration of `GitIgnorePattern` as "gitwildmatch"
util._registered_patterns['gitwildmatch'] = GitWildMatchPattern
# Force registration of `GitIgnorePattern` as "gitignore"
util._registered_patterns['gitignore'] = GitIgnorePattern
